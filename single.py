...
def getSpotLocation(position, board):
    ...
    for row in board:
        for spot in row:
            if(spot.x <= x <= spot.x+ spot.width and spot.y <= y <= spot.y+ spot.width):
...
def getLineInformation(board):
    for i in range(3):
        ...
        if abs(sum_h) == 3 or abs(sum_v) == 3 or abs(sum_d1) == 3 or abs(sum_d2) == 3:
            if abs(sum_v) == 3:
                return (board[i][0].x + 100, board[i][0].y + 100), (board[i][2].x + 100, board[i][2].y+100)
            if abs(sum_h) == 3:
                return (board[0][i].x + 100, board[0][i].y + 100), (board[2][i].x + 100, board[2][i].y+100)
            if abs(sum_d1) == 3:
    ...
    while run:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
            if AI > 0:
                row, column = best(board, computer_move, lambda: result(board))
            ...
            if AI < 0 and pygame.mouse.get_pressed()[0]:
                ...
                try:
                    ...
                    print(e)
                    print("Please click inside the spot")
